 ...
 *loginInfo : 当前用户身份
*/
var selToID,loginInfo,accountMode,accountType,sdkAppID,avChatRoomId,selType,selSess,selSessHeadUrl;
...
function init(opts) {
  ...
  selType = opts.selType; //群聊类型
  selToID = opts.selToID;
}
/* 
 * 《2》sdk登录
 * @param userInfo     : 当前用户身份
 * @param listeners    ：监听事件
 * @param options      : 其他对象
 * @param avChatRoomId : 聊天群id
*/
function sdkLogin(userInfo, listeners, options, avChatRoomId) {
  //@param identifierNick为登录用户昵称(没有设置时，为帐号),无登录态时为空
  webim.login(userInfo, listeners, options,function (identifierNick) {
...
function setProfilePortrait(options,callback){
  webim.setProfilePortrait(options,function(res){
...
function onSendMsg(msg, callback) {
  ...
  if (!selSess) {
    //聊天对象-消息所属的会话
    //webim.Session(群聊类型,群ID,群名称,群头像URL,最新消息的时间戳,最新消息的序列号)
  ...
  //消息的描述类
  // webim.Msg('聊天对象','消息是否自己发送','消息序列号','消息随机数','消息时间戳','消息发送者帐号','消息类型','消息发送者昵称')
...
function convertGroupTipMsgToHtml(content) {
    ...
    switch (opType) {
        case webim.GROUP_TIP_TYPE.JOIN://加入群
            ...
            arruserinfo = content['userinfo']
            for (var m in userIdList) {
                text += arruserinfo[m].NickName + ",";
